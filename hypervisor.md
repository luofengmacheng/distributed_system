...
### 2.1 与环境通信(Interaction with an Environment)

环境的状态会受到执行I/O指令的影响。我们必须保证，环境所看到的I/O指令序列同运行在单处理器上的一样，虽然，我们容1个错的虚拟机是用两个处理器构建的。这个问题可以分成两种情况：

* epochs过程中，主处理器没有故障
* epochs过程中，主处理器发生了故障，被称为failover epochs

这两种情况都可以解决，根据P7，当主虚拟机在某个epoch过程中故障了，备份虚拟机会在接下来的epoch到来使，被提升为主虚拟机。因此，每个epoch过程中，要么主虚拟机正常工作，要么主虚拟机故障了。

第一种情况很简单。在这些epochs中，备份虚拟机的I/O指令被备份hypervisor阻塞。回忆P5，备份虚拟机不会开始epoch E，直到主虚拟机完成了那个epoch(因为要等待消息[end, E])。因此，备份hypervisor在epoch E开始时就知道主hypervisor已经完成了这个epoch，而没有发生故障。于是，在这个epoch中，备份hypervisor能够阻塞备份虚拟机的I/O操作。最后，环境只能看到每条I/O指令在主虚拟机上的执行。

第二种情况是有问题的，因为它会造成两个未解决的常见问题。没有协议能够通知备份hypervisor，主虚拟机在故障之前是否想要执行一条特定的I/O指令。这是因为，我们假设主hypervisor用来初始化I/O的操作跟用来与备份hypervisor通信的操作不同。在协议中，在发送一条I/O指令之后，主就会与备份hypervisor通信，在执行I/O之后但是没有通信时，主故障了，那么，备份hypervisor就会认为I/O没有开始；在发送I/O指令之前，会发送通知，发送通知之后执行I/O之前主故障了，备份hypervisor就会认为发送了I/O指令。

因此，无法保证在一个故障的epoch过程中，发送的每个I/O指令在外界看来只发生了一次。我们的解决方案是，利用这个事实：I/O设备容易受到暂时故障的影响，设备驱动已经处理了这些故障。假设所有的I/O设备遵从下列接口：

IO1：如果发送并执行了I/O指令，然后，执行这条指令的处理器接下来会收到一个completion中断。

IO2：如果发送I/O指令的处理器受到一个uncertain中断，那么，I/O可能执行了，也可能还没有执行。

HP的PA-RISC使用的SCSI总线和DEC的Alpha的I/O架构都满足这些要求，而且，I/O设备还可以通过通信网络访问。使用SCSI总线协议，CHECK_CONDITION命令完成中断状态跟IO2的uncertain中断有相同的语义。

满足IO1和IO2的I/O设备的操作系统驱动可能会重试I/O指令。特别地，无论何时收到一个uncertain中断，未决的I/O指令必须重试。因此，环境(例如，I/O设备)必须容忍I/O指令的重试。并且，我们可以用它来处理主虚拟机发送的I/O操作，在主虚拟机故障之前，向备份hypervisor发送一个既不是completion中断，也不是uncertain中断的中断。

P8：当备份虚拟机完成了failover epoch(例如，在备份虚拟机被提升为主虚拟机之前)，备份hypervisor为每个I/O操作生成一个uncertain中断。

P8能够使确定的I/O指令重试。然而，考虑到环境，这种重试可能是造成I/O设备返回uncertain中断的暂时时间的结果。因此，环境可以看到一系列指令，这些指令同单处理器上观察到的一样。毫无悬念，实际中使用的I/O设备-磁盘和网络适配器-也拥有这样的功能。
...
hypervisor不仅要实现虚拟机(虚拟机的指令集架构跟裸机类似，让人无法区分)，还要高效地实现。虚拟机执行指令的速度应该跟硬件接近。典型地，可以利用双模处理器架构实现高效，双模架构中，以管理者模式运行允许执行特权指令和非特权指令，以用户模式运行只允许执行非特权指令。hypervisor运行在管理者模式，可以收到任何中断或者陷入的控制。其它的软件，包括虚拟机的操作系统内核，都运行在用户模式。无论何时，运行在虚拟管理者模式的虚拟机想要指令特权指令，就会发生特权陷入，hypervisor会模拟那条指令。

然而，为HP的PA-RISC实现hypervisor不是很直接的。指令集架构的两个方面妨碍了有效的虚拟化：内存架构和处理器特权等级。幸运的是，这可以通过构造一个仅支持有限制的虚拟机(它足以运行HP的UNIX系统)的hypervisor实现。我们的hypervisor包含大约2.4万行代码(其中，5千行是汇编语言，其它的是C语言)。

**内存架构** 在HP的PA-RISC架构中，地址翻译使用的是一些空间寄存器(space register)。空间寄存器定义了逻辑地址段。读取空间寄存器的指令是非特权指令，甚至使用非特权指令可以写空间寄存器的一部分。这样的话，hypervisor就不能拦截对空间寄存器的非特权访问，这使得支持多虚拟机是很困难的。特别地，当虚拟机改变了一个空间寄存器，没有调用hypervisor，那么，虚拟机可能会推断，存在另外一个空间寄存器的值。
